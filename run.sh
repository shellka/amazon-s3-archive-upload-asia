...
if [ ! -n "$WERCKER_S3_ARCHIVE_UPLOAD_BUCKET4" ]; then
  #set default bucket
  export WERCKER_S3_ARCHIVE_UPLOAD_BUCKET4="wercker-deployments"
fi

if [ ! -n "$WERCKER_S3_ARCHIVE_UPLOAD_REGION4" ]; then
...
echo "region = $WERCKER_S3_ARCHIVE_UPLOAD_REGION4" >> $HOME/.aws/config
...
export AWS_DEFAULT_REGION=$WERCKER_S3_ARCHIVE_UPLOAD_REGION4
...
aws s3 cp --acl private $AWS_APP_FILENAME s3://$WERCKER_S3_ARCHIVE_UPLOAD_BUCKET4
